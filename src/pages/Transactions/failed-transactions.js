...
const FailedTransactions = ()  => {
  ...
  const loadTransactions = async() => {
     await  axiosApi.get(`${API_URL}/api/v1/transactions/failed`).then((response) => {
         setTransactions(response.data)
      }).catch((error) => {
        console.log(error)
        toast("oops, something went wrong fetching failed transactions");
      })
  }
...
export default withRouter(FailedTransactions);