...
const TransactionList = ()  => {
  ...
  const loadTransactions = async () => {
      await axiosApi.get(`${API_URL}/api/v1/transactions/approved`).then((response) => {
        ...
        formatter: (cellContent, transaction) => (
          <>
            <h5 className="font-size-14 mb-1">
              ...
              transaction.type == "WITHDRAWAL" || transaction.type == "AIRTIME" || transaction.type == "DATA" || transaction.type == "FUNDING" ||  transaction.type == "ELECTRICITY" || transaction.type == "CABLE"  || transaction.type == "SIGNUP_BONUS" ? (
                 <>
                   &#8358;{transaction.amount}
                 </>
              ) 
    ...
    <React.Fragment>
     ...
     <ConfirmModal
      ...
      <div className="page-content">
        ...
        <Container fluid>
          {/* Render Breadcrumbs */}
          <Breadcrumbs title="transactions" breadcrumbItem="Succesfull Transactions" />
            <Col xl="12">
              <Row>
                {/* Reports Render */}
