...
const loadWallets = () => {
  axiosApi.get(`${API_URL}/api/v1/users/user/crypto-wallets/${id}`).then((res) => {
      setWallets(res.data)
  }).catch(() => {
...
const loadUserProfile = () => {
    axiosApi.get(`${API_URL}/api/v1/users/profile/${id}`).then((res) => {
...
function formatDate(date) {
                        ...
                        >
                          {toolkitProps => (
                            <React.Fragment>
                              ...
                              <Col sm="4">
                                    <h5>{userProfile?.name}s Crypto Wallets</h5>
                                </Col>
 ...
 <Modal isOpen={modal} toggle={onCloseClick} centered={true}>
    <ModalHeader toggle={onCloseClick} tag="h4" style={{ paddingLeft: "40px"}} >
       {userProfile?.name}s Crypto Wallet Details
    </ModalHeader>
